...
field = exp(1i*2*pi*(indexDiff-indexEnv)*diffuser/lambda);
inputmask = exp(1i*2*pi*z0/lambda)/(1i*lambda*z0) * exp(1i*pi*(xmesh.^2+ymesh.^2)/(lambda*z0));

[sensorplane, X, Y] = propagate_field(x, y, field, z1, lambda, inputmask);

figure(1);
imagesc(X(1,:),Y(:,1),abs(sensorplane).^2);
